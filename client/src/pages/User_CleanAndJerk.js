...
class User_CleanAndJerk extends Component {
  state = {
  ...
  loadUser = () => {
    ...
    API.getUser(this.props.match.params.id)
      .then(res => {
        ...
        this.setState({ all: res.data[0].clean_and_jerk[0].history })
      }).then(() => {
  ...
  loadRecent = () => {
      ...
      API.getUser(this.props.match.params.id)
        .then(res => {
            this.setState({ all: res.data[0].clean_and_jerk[0].history.pop() })
    ...
    return (
      <Container fluid>
        <Row>
          <Col size="md-6 sm-12">
            <Jumbotron>
              <h1>User Clean and Jerk History</h1>
            </Jumbotron>
            <br></br>
          ...
          <Col size="md-6 sm-12">
          <MyWindowPortal>
          <h2>Clean and Jerk Progress (y, pounds) and its First Derivative (yDeriv, pounds per day)</h2>
...
export default User_CleanAndJerk;
