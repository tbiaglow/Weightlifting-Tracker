...
class User_Snatch extends Component {
  state = {
  ...
  loadUser = () => {
    ...
    API.getUser(this.props.match.params.id)
      .then(res => {
        ...
        this.setState({ all: res.data[0].snatch[0].history })
      }).then(() => {
  ...
  loadRecent = () => {
      ...
      API.getUser(this.props.match.params.id)
        .then(res => {
            this.setState({ all: res.data[0].snatch[0].history.pop() })
    ...
    return (
      <Container fluid>
        <Row>
          ...
          <Col size="md-6 sm-12">
          <MyWindowPortal>
          <h2>Snatch Progress (y, pounds) and its First Derivative (yDeriv, pounds per day)</h2>
