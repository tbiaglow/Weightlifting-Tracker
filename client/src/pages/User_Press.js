...
class User_Press extends Component {
  state = {
  ...
  loadUser = () => {
    ...
    API.getUser(this.props.match.params.id)
      .then(res => {
        ...
        this.setState({ all: res.data[0].press[0].history })
      }).then(() => {
    ...
    return (
      <Container fluid>
        <Row>
          ...
          <Col size="md-6 sm-12">
          <MyWindowPortal>
          <h2>Press Progress (y, pounds) and its First Derivative (yDeriv, pounds per day)</h2>
          ...
          <MyWindowPortal>
          <h2>Press Progress Relative to Bodyweight (y, pounds) and its First Derivative (yDeriv, pounds per day)</h2>
          <ResponsiveContainer width='99%' height={500} >
          <LineChart
